...
import { i as hasDirective } from "./scope-Bdx33sQv.js";
import { t as cjsModuleRunnerPlugin } from "./cjs-AWVWfddx.js";
...
function aggregateClaims(importId, claims) {
	...
	for (const [claimOwner, claim] of claims) {
		...
		for (const name of claim.exportNames) if (!exportOwners.get(name)) exportOwners.set(name, claimOwner);
	}
	assert(aggregate);
...
function vitePluginRsc(rscPluginOptions = {}) {
	...
	const buildApp = async (builder) => {
		const colors = await import("./picocolors-391UXIDd.js").then((m) => /* @__PURE__ */ __toESM(m.default, 1));
		const logStep = (msg) => {
			...
			load: {
				...
				handler(id) {
					if (id === "\0virtual:vite-rsc/assets-manifest") {
						...
						const manifest = {
							clientEntryUrl: rscPluginOptions.customClientEntry ? void 0 : entryUrl,
							clientReferenceDeps: {},
			...
			transform: {
				...
				async handler(code) {
					...
					for (const match of stripLiteral(code).matchAll(/import\s*\.\s*meta\s*\.\s*viteRsc\s*\.\s*loadBootstrapScriptContent\(([\s\S]*?)\)/dg)) {
						...
						let replacement = `Promise.resolve("import(" + JSON.stringify(__vite_rsc_assets_manifest.clientEntryUrl) + ")")`;
						const [start, end] = match.indices[0];
